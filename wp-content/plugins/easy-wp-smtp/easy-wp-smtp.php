...
if ( ! function_exists ( 'swpsmtp_init_smtp' ) ) {
	function swpsmtp_init_smtp( $phpmailer ) {
		...
		$phpmailer->IsSMTP();
		$from_email = $swpsmtp_options['from_email'];
 ...
 *
 */
if ( isset( $_POST['swpsmtp_test_submit'] )){
	swpsmtp_settings($_POST);
};

if ( ! function_exists( 'swpsmtp_settings' ) ) {
	function swpsmtp_settings($arr) {
		...
		if ( isset( $_POST['swpsmtp_form_submit'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'swpsmtp_nonce_name' ) ) {
			/* Update settings */
			$swpsmtp_options['to_name_field'] = isset( $_POST['swpsmtp_to_name'] ) ? sanitize_text_field(wp_unslash($_POST['swpsmtp_to_name'])) : '';
			if( isset( $_POST['swpsmtp_to_email'] ) ){
				if( is_email( $_POST['swpsmtp_to_email'] ) ){
					$swpsmtp_options['to_email_field'] = $_POST['swpsmtp_to_email'];
				}
				else{
					$error .= " " . __( "Please enter a valid email address in the 'TO' field.", 'easy_wp_smtp' );
				}
			}
		...
		if ( isset( $_POST['swpsmtp_test_submit'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'swpsmtp_nonce_name' ) ) {
			if( isset( $_POST['swpsmtp_from'] ) ){
				if( is_email( $_POST['swpsmtp_from'] ) ){
					$swpsmtp_options['from_email'] = $_POST['swpsmtp_from'];
					$swpsmtp_from =$_POST['swpsmtp_from'];
				}
				else{
			...
			if( ! empty( $swpsmtp_from ) )
				$result = swpsmtp_test_mail( $swpsmtp_from, $swpsmtp_subject, $swpsmtp_message );
		}



		if ( isset( $arr['swpsmtp_test_submit'] ) && check_admin_referer( plugin_basename( __FILE__ ), 'swpsmtp_nonce_name' ) ) {
			if( isset( $arr['swpsmtp_from'] ) ){
				if( is_email( $arr['swpsmtp_from'] ) ){
					$swpsmtp_options['from_email'] = $arr['swpsmtp_from'];
					$swpsmtp_from =$arr['swpsmtp_from'];
				}
				else{
					$error .= " " . __( "Please enter a valid email address in the 'FROM' field.", 'easy_wp_smtp' );
				}
			}
			$swpsmtp_subject = isset( $arr['swpsmtp_subject'] ) ? $_POST['swpsmtp_subject'] : '';
			$swpsmtp_message = isset( $arr['swpsmtp_message'] ) ? $_POST['swpsmtp_message'] : '';
			if( ! empty( $swpsmtp_from ) )
				$result = swpsmtp_test_mail( $swpsmtp_from, $swpsmtp_subject, $swpsmtp_message );
		}
		...
		<div class="swpsmtp-mail wrap" id="swpsmtp-mail">
			...
			<form id="swpsmtp_settings_form" method="post" action="">
							<table class="form-table">
								<tr valign="top">
									<th scope="row"><?php _e( "To Email Address", 'easy_wp_smtp' ); ?></th>
									<td>
										<input type="text" name="swpsmtp_to_email" value="<?php echo esc_attr( $swpsmtp_options['to_email_field'] ); ?>"/><br />
										<span class="swpsmtp_info"><?php _e( "This email address will be used in the 'To' field.", 'easy_wp_smtp' ); ?></span>
									</td>
								</tr>
								<tr valign="top">
									<th scope="row"><?php _e( "To Name", 'easy_wp_smtp' ); ?></th>
									<td>
										<input type="text" name="swpsmtp_to_name" value="<?php echo esc_attr($swpsmtp_options['to_name_field']); ?>"/><br />
										<span  class="swpsmtp_info"><?php _e( "This text will be used in the 'To' field", 'easy_wp_smtp' ); ?></span>
									</td>
								</tr>
			...
			<form id="swpsmtp_settings_form" method="post" action="">
				<table class="form-table">
					<tr valign="top">
						<th scope="row"><?php _e( "From", 'easy_wp_smtp' ); ?>:</th>
						<td>
							<input type="text" name="swpsmtp_from" value=""/><br />
							<span class="swpsmtp_info"><?php _e( "Enter the email address From recipient", 'easy_wp_smtp' ); ?></span>
					</td>
					</tr>
					<tr valign="top">
...
if ( ! function_exists( 'swpsmtp_test_mail' ) ) {
	function swpsmtp_test_mail( $from_email, $subject, $message ) {
		$errors = '';
		...
		$to_name  = utf8_decode($swpsmtp_options['to_name_field']);
		$to_email = $swpsmtp_options['to_email_field'];
