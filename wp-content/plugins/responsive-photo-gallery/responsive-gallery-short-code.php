...
function image_gallery_premium_short_code( $Id ) {
    ...
    ?>
    <div  class="gal-container gallery1">
    <?php while ( $loop->have_posts() ) : $loop->the_post();?>
        <!--get the post id-->
        <?php $post_id = $Id['id']; ?>
		<div id="gal-container-<?php echo $post_id; ?>" style="display: block;overflow-x: hidden">
			<?php if($WL_Gallery_Title==""){ $WL_Gallery_Title == "yes"; } if($WL_Gallery_Title == "yes") { ?>
			<!-- gallery title-->
			<div class="rpg-gal-title" id="<?php echo get_the_title($post_id); ?>">
			</div>
			<?php } ?>
			<!-- gallery photos-->
			<div style="">
				...
				foreach($rpg_all_photos_details as $rpg_single_photos_detail) {
					...
					?>
					<div class="<?php echo $WL_Gallery_Layout; ?> col-sm-6 wl-gallery <?php echo get_the_title($post_id); ?>" >
						<div class="b-link-<?php echo $WL_Hover_Animation; ?> b-animate-go col-md-10">
							<img src="<?php echo $url; ?>" class="img-responsive img-rounded" style="margin: 0 auto;">
							<div class="b-wrapper">
									...
									<div class="team-detail">
										...
										<p class="b-from-right b-animate b-delay03">
											<?php
												...
												$pro = strrchr($url,".");
												$num = sizeof($file_name);
												unset($file_name[$num-1]);
												$url = implode("-",$file_name);
												$url .=$pro;
											?>
											<a type="button" href="<?php echo $url; ?>" data-title="<?php echo $name; ?>" data-lightbox="group">
